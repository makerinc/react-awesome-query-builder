...
export default class Field extends Component {
  ...
  filterOption = (input, option) => {
    ...
    if (option.props.groupLabel) {
      isInGroupLabel =
        option.props.groupLabel.toLowerCase().indexOf(input.toLowerCase()) >= 0;
    }
  ...
  buildSelectItems(
    ...
    return fieldKeys.map(fieldKey => {
      ...
      if (field.type == "!struct") {
        let subpath = (path ? path : []).concat(fieldKey);
        return (
          <OptGroup key={prefix + fieldKey} label={label}>
  ...
  buildMenuToggler(label, fullLabel, customLabel) {
    ...
    var toggler = (
      <Button size={this.props.config.settings.renderSize || "small"}>
    ...
    if (fullLabel && fullLabel != label) {
      toggler = (
        <Tooltip placement="top" title={fullLabel}>
          {toggler}
        </Tooltip>
      );
    }
  ...
  render() {
    if (this.props.renderAsDropdown) return this.renderAsDropdown();
    else return this.renderAsSelect();
  }
  ...
  renderAsSelect() {
    ...
    let selectedFieldPartsLabels = getFieldPathLabels(
      this.props.selectedField,
      this.props.config
    );
    let selectedFieldFullLabel = selectedFieldPartsLabels
      ? selectedFieldPartsLabels.join(
          this.props.config.settings.fieldSeparatorDisplay
        )
      : null;
    let placeholder = !isFieldSelected
      ? this.props.config.settings.fieldPlaceholder
      : null;
    let fieldDisplayLabel = isFieldSelected
      ? this.getFieldDisplayLabel(this.curField(), this.props.selectedField)
      : null;
    ...
    let fieldSelectItems = this.buildSelectItems(
      fieldOptions,
      undefined,
      undefined,
    ...
    let fieldSelect = (
      <Select
        dropdownAlign={
          dropdownPlacement ? BUILT_IN_PLACEMENTS[dropdownPlacement] : undefined
        }
        dropdownMatchSelectWidth={false}
        style={{
          width:
            isFieldSelected && !customProps.showSearch ? null : selectWidth + 36
        }}
        ref="field"
  ...
  renderAsDropdown() {
    let fieldOptions = this.props.config.fields;
    let selectedFieldKeys = getFieldPath(
      this.props.selectedField,
      this.props.config
    );
    let selectedFieldPartsLabels = getFieldPathLabels(
    ...
    let selectedFieldFullLabel = selectedFieldPartsLabels
      ...
      : null;
    let placeholder =
      this.curFieldOpts().label || this.props.config.settings.fieldPlaceholder;
    let customProps = this.props.customProps || {};
    ...
    let fieldToggler = this.buildMenuToggler(
      placeholder,
      selectedFieldFullLabel,
      this.curFieldOpts().label2
    );
