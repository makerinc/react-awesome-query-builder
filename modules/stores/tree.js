...
const removeGroup = (state, path, config) => {
  ...
  let canLeaveEmpty =
    isEmptyGroup && config.settings.canLeaveEmptyGroup && !isEmptyRoot;
...
const moveItem = (state, fromPath, toPath, placement, config) => {
  ...
  let isSourceInsideTarget =
    targetPath.size < sourcePath.size &&
    JSON.stringify(targetPath.toArray()) ==
      JSON.stringify(sourcePath.toArray().slice(0, targetPath.size));
  let isTargetInsideSource =
    targetPath.size > sourcePath.size &&
    JSON.stringify(sourcePath.toArray()) ==
      JSON.stringify(targetPath.toArray().slice(0, sourcePath.size));
 ...
 */
export const _getNewValueForFieldOp = function(
  config,
  oldConfig = null,
  current,
  newField,
  ...
  const currentOperatorConfig = getOperatorConfig(
    oldConfig,
    currentOperator,
    currentField
  );
  ...
  const currentWidgets = Array.from(
  ...
  let canReuseValue =
    ...
    JSON.stringify(currentWidgets.slice(0, commonWidgetsCnt)) ==
      JSON.stringify(newWidgets.slice(0, commonWidgetsCnt));
  if (canReuseValue) {
  ...
  newValue = new Immutable.List(
    Array.from({ length: operatorCardinality }, (_ignore, i) => {
      ...
      if (canReuseValue) {
        ...
        firstWidgetConfig &&
        firstWidgetConfig.defaultValue !== undefined
      ) {
...
const _validateValue = (
  ...
  let fieldWidgetDefinition = omit(
    getFieldWidgetConfig(config, field, operator, w, vSrc),
    ["factory", "formatValue"]
  );
  let isValid = true;
  if (v != null) {
    ...
    if (vSrc == "field") {
      ...
      let iterableValues =
        typeof fieldConfig.listValues === "function"
          ? fieldConfig.listValues()
          : fieldConfig.listValues;
...
const setField = (state, path, newField, config) => {
  ...
  return state.updateIn(expandTreePath(path, "properties"), map =>
    map.withMutations(current => {
      ...
      const lastOp =
        newFieldConfig &&
        newFieldConfig.operators.indexOf(currentOperator) !== -1
          ? currentOperator
          : null;
      let newOperator = null;
      const availOps = getOperatorsForField(config, newField);
      if (availOps.length == 1) newOperator = availOps[0];
      ...
      let newOperatorOptions = canReuseValue
